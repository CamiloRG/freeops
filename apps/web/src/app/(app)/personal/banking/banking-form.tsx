...
export function BankingForm({ current }: { current: BankingMasked | null }) {
  ...
  return (
    <SummaryEditCard
      ...
      description="Used to receive payments. Your full account number is encrypted at rest and never shown again after saving — only the last 4 digits are displayed."
      editing={editing}
      ...
      summary={
        saved ? (
          <SummaryGrid>
            <SummaryField label="Bank" value={saved.bankName} />
            <SummaryField label="Account type" value={<span className="capitalize">{saved.accountType}</span>} />
            <SummaryField label="Account number" value={saved.accountNumberMasked} />
            <SummaryField label="Account holder" value={saved.accountHolderName} />
          </SummaryGrid>
        ) : (
          <p className="text-sm text-muted-foreground">No banking details saved yet.</p>
        )
      }
    >
